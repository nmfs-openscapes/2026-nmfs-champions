
<a align="left" href="https://github.com/nmfs-openscapes/2026-nmfs-champions"><img src="https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png" width="35px"/></a>

# 2026 NMFS Openscapes Champions

Welcome to the 2026 NMFS Openscapes Champions Program for NOAA Fisheries
(National Marine Fisheries Service, NMFS)! This is the website for the
three NOAA Fisheries Cohorts running concurrently in Fall 2026 with
teams and individuals from across NMFS centers and offices. To learn
more about what to expect (the purpose, outcomes, and process of our
Cohort) and to browse the open lesson materials, please see the
[Champions Program Lesson Series](https://openscapes.github.io/series).

<img src="horst-champions-trailhead.png" align="right" width="450"/>

## Cohort Agendas

Each Cohort will meet via Zoom five times over two months for 1.5 hours
each. Agendas are accessible to Cohort participants in our Cohort Google
Drive Folders (links to come); they are an archive of our live google-doc’ing.

**Cohort A**: Sep 22, Oct 6, 20, Nov 3, 17. Tuesdays 1:00pm - 2:30pm PT.  

**Cohort B**: Sep 23, Oct 7, 21, Nov 4, 18. Wednesdays 10:00 - 11:30am PT.  

**Cohort C**: Sep 23, Oct 7, 21, Nov 4, 18. Wednesdays 1:00 - 2:30pm PT.



| Cohort Call Topics | Lesson Series Chapters | Seaside Chat Topics |
|----|----|----|
| 1\. Openscapes mindset | [mindset](https://openscapes.github.io/series/mindset), [better science for future us](https://openscapes.github.io/series/core-lessons/better-science) | Pathway trailhead; GitHub accounts |
| 2\. GitHub for documentation & project management | [publishing](https://openscapes.github.io/series/github-pub), [project management](https://openscapes.github.io/series/github-issues) | team awareness & collaboration strategies |
| 3\. Cloud strategies | cloud migration approaches at NOAA Fisheries | cloud migration |
| 4\. Open communities in the age of AI, & data strategies for future us | [open communities](https://openscapes.github.io/series/core-lessons/communities), [data archiving and preservation](https://openscapes.github.io/series/core-lessons/data-strategies.html) | prepare Pathways share |
| 5\. Pathways share and next steps | [pathways](https://openscapes.github.io/series/core-lessons/pathways) |  |

<!---TODO: link to issues labeled 'digest'. 
Note result will be empty until after first call digest is posted.
&#10;[**Cohort Call Digests**](issues)
--->

## Coworking times (optional)

Coworking sessions are where all participants come to get their own work
done at the same time together. Sometimes, this means quiet work with
check-ins to break up focused work and get feedback, and sometimes this
involves asking questions and screensharing to learn and problem solve.
We will make breakout rooms so people working on related themes can meet
and learn from each other.

- **Dates:** Tuesdays Sep 29, Oct 13, 27, Nov 10
- **Times:** 12:00 - 1:30pm PT
- **Location:** remotely, via Zoom

## Participating teams and individuals

We’ll add some brief information about participating teams and
individuals. Please add any edits directly (we’ll learn how in our
GitHub Clinic!)

## Our Team

Julie Lowndes ([@jules32](https://github.com/)), Openscapes  
Stefanie Butland
([@stefaniebutland](https://github.com/stefaniebutland)), Openscapes  
Ileana Fenwick ([@IleanaF](https://github.com/IleanaF)), Openscapes  
Eli Holmes ([@eeholmes](https://github.com/eeholmes)), NMFS Open
Science  
Jon Peake ([@jonpeake](https://github.com/jonpeake)), NMFS Open
Science  
[NOAA Fisheries Openscapes
Mentors](https://nmfs-openscapes.github.io/mentors/)

## More about Openscapes and the Champions program:

- [**Our path to better science in less time using open data science
  tools**](https://www.nature.com/articles/s41559-017-0160) (Lowndes et
  al. 2017) - This describes a marine science team’s transition to open
  collaborative teamwork. It was the original inspiration for creating
  the Champions Program and heavily influences the Core Lessons. We’ll
  ask that everyone participating reads it before our first Cohort Call.
- [**Supercharge your research: a ten-week plan for open data
  science**](https://openscapes.github.io/supercharge-research/)
  (Lowndes et al. 2019) - This was co-authored with the inaugural
  Champions Cohort, capturing the most valuable take-aways for marine
  and environmental science early career faculty.
- [**Shifting institutional culture to develop climate solutions with
  Open
  Science**](https://onlinelibrary.wiley.com/doi/10.1002/ece3.11341)
  (Lowndes et al. 2024) - This was co-authored by Openscapes mentors
  across organizations – including NASA Earthdata, NOAA Fisheries, EPA,
  California Water Boards, Pathways to Open Science, Fred Hutch Cancer
  Center.
